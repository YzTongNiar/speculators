...
class VwnEagle3PreVwnLayerV1(nn.Module):
    ...
    def __init__(
        ...
        vwn: VwnConfig,
        norm_eps: float,
        norm_class: type[nn.Module] = Qwen3RMSNorm,  # 支持 LlamaRMSNorm / Qwen3RMSNorm
    ):
        super().__init__()
        d_prime = vwn.expanded_hidden_size(hidden_size)

        self.input_layernorm = norm_class(hidden_size, eps=norm_eps)  # E-norm1
        self.hidden_norm = norm_class(hidden_size, eps=norm_eps)       # H-norm1
    ...
    def forward(
        ...
        fused = torch.cat([e_normed, h_normed], dim=-1)   # [B, T, 2D]
        fused = self.fc(fused)                             # [B, T, D]
        return self.upward(fused)                          # [B, T, D']
...
class VwnEagle3FirstLayerMixin:
    ...
    def _patch_vwn_eagle3_projections(
        ...
        self.pre_vwn_layer = pre_vwn_layer_class(hidden_size, vwn, norm_eps, norm_class)

        # ── A2: D' → D + D'（split 后：前 D 送 Attn，后 D' 作为新残差） ──
        self.downward_and_forgot = nn.Linear(d_prime, hidden_size + d_prime, bias=False)

        # ── norm0: Attn 输入前归一化（工作在 D 空间），架构对齐 ────────────
        self.pre_attention_layernorm = norm_class(hidden_size, eps=norm_eps)

        # ── B1: D → D'（将 Attn 输出扩维并加到残差上） ────────────────────
        self.upward_after_attn = nn.Linear(hidden_size, d_prime, bias=False)

        # ── A3: 同 A2，但权重独立 ──────────────────────────────────────────
        self.downward_and_forgot_after_attn = nn.Linear(
            d_prime, hidden_size + d_prime, bias=False
        )
        
        # ── B2: D → D'（将 FFN 输出扩维并加到残差上） ─────────────────────
        self.upward_after_mlp = nn.Linear(hidden_size, d_prime, bias=False)

        # ── A4: D' → D（层间数据连接） ────────────────────────────────────
        self.downward = nn.Linear(d_prime, hidden_size, bias=False)
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],  # type: ignore[valid-type]
        ...
        daf_out = self.downward_and_forgot(vwn_states)          # [B, T, D + D']
        ...
        vwn_states = vwn_residual + self.upward_after_attn(attn_out)
        # vwn_states: [B, T, D']

        # ── 6. A3：D' → D（FFN 输入）+ D'（VWN 残差） ────────────────────
        daf2_out = self.downward_and_forgot_after_attn(vwn_states)
        ...
        vwn_states = vwn_residual2 + self.upward_after_mlp(mlp_out)
        # vwn_states: [B, T, D']

        # ── 9. A4：降回 D，传给下一层 ─────────────────────────────────────
        # TODO: print hidden_states after MLP
        return self.downward(vwn_states)
        # output: [B, T, D]
...
model_classes: dict[str, base_components.ModelComponents] = {
    ...
    "vwn_llama": base_components.override_components(
        "llama", first_layer_class=LlamaDecoderVwnEagle3FirstLayer,
    )
}
